...
> [`terraform-google-modules/terraform-google-sql-db` → `modules/mysql`](https://github.com/terraform-google-modules/terraform-google-sql-db/tree/master/modules/mysql)
> (published on the Terraform Registry as `terraform-google-modules/sql-db/google//modules/mysql`).
> Do **not** modify the `.tf` files locally; sync from upstream instead:
>
> ```bash
> cp /path/to/terraform-google-sql-db/modules/mysql/*.tf \
>    ./modules/cloud-sql-mysql/
> ```
>
...
| <a name="input_auto_upgrade_enabled"></a> [auto\_upgrade\_enabled](#input\_auto\_upgrade\_enabled) | Enables Automatic Version Upgrade for MYSQL\_8\_0 based minor versions. The database\_version must be MYSQL\_8\_0\_35 or higher. Once enabled, this cannot be unset. | `bool` | `null` | no |
...
| <a name="input_backup_configuration"></a> [backup\_configuration](#input\_backup\_configuration) | The backup\_configuration settings subblock for the database settings | <pre>object({<br/>    binary_log_enabled             = optional(bool, false)<br/>    enabled                        = optional(bool, false)<br/>    start_time                     = optional(string)<br/>    location                       = optional(string)<br/>    transaction_log_retention_days = optional(string, 7)<br/>    retained_backups               = optional(number, 8)<br/>    retention_unit                 = optional(string)<br/>  })</pre> | `{}` | no |
...
| <a name="input_disk_size"></a> [disk\_size](#input\_disk\_size) | The disk size (in GB) for the master instance | `number` | `10` | no |
...
| <a name="input_insights_config"></a> [insights\_config](#input\_insights\_config) | The insights\_config settings for the database. | <pre>object({<br/>    enhanced_query_insights_enabled = bool<br/>    query_plans_per_minute          = number<br/>    query_string_length             = number<br/>    record_application_tags         = bool<br/>    record_client_address           = bool<br/>  })</pre> | `null` | no |
...
| <a name="input_ip_configuration"></a> [ip\_configuration](#input\_ip\_configuration) | The ip\_configuration settings subblock | <pre>object({<br/>    authorized_networks                           = optional(list(map(string)), [])<br/>    ipv4_enabled                                  = optional(bool, true)<br/>    private_network                               = optional(string)<br/>    ssl_mode                                      = optional(string)<br/>    server_ca_mode                                = optional(string)<br/>    allocated_ip_range                            = optional(string)<br/>    enable_private_path_for_google_cloud_services = optional(bool, false)<br/>    psc_enabled                                   = optional(bool, false)<br/>    psc_allowed_consumer_projects                 = optional(list(string), [])<br/>  })</pre> | `{}` | no |
...
| <a name="input_password_validation_policy_config"></a> [password\_validation\_policy\_config](#input\_password\_validation\_policy\_config) | The password validation policy settings for the database instance. | <pre>object({<br/>    enable_password_policy      = bool<br/>    min_length                  = optional(number)<br/>    complexity                  = optional(string)<br/>    disallow_username_substring = optional(bool)<br/>    reuse_interval              = optional(number)<br/>  })</pre> | `null` | no |
...
| <a name="input_tier"></a> [tier](#input\_tier) | The tier for the master instance, for ADC its default value will be db-perf-optimized-N-8 which is tier value for edition ENTERPRISE\_PLUS, if user wants to change the edition, they should chose compatible tier. | `string` | `"db-n1-standard-1"` | no |
