...
# The upstream module does not cover BigQuery dataset bindings. This variable
# adds support for granting roles to this service account on BQ datasets,
# implemented in iam-bigquery.tf via google_bigquery_dataset_iam_member.
variable "iam_bigquery_dataset_roles" {
  description = "BigQuery dataset roles granted to this service account, by 'project_id/dataset_id' key. Non-authoritative."
  type        = map(list(string))
  default     = {}
  nullable    = false
  validation {
    condition = alltrue([
      for k in keys(var.iam_bigquery_dataset_roles) :
      length(split("/", k)) == 2
    ])
    error_message = "Each key in iam_bigquery_dataset_roles must be in the format 'project_id/dataset_id'."
  }
}
